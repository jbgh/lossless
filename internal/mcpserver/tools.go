	...
	return []map[string]any{
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"session_id":     map[string]any{"type": "string", "description": "Harness session id when this prompt shows one (a subagent or workflow child sends its own). Otherwise omit it: Claude Code, Pi, and OpenCode fill it; Grok and Codex have no fill. Do not invent one; never send default."},
		...
		},
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"session_id":     map[string]any{"type": "string", "description": "Harness session id when this prompt shows one, so the remember lands on this session's tape. Otherwise omit it (Claude Code, Pi, and OpenCode fill it)."},
				},
			},
		},
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"project":        map[string]any{"type": "string"},
					"workspace_root": map[string]any{"type": "string"},
					"session_id":     map[string]any{"type": "string", "description": "Harness session id when this prompt shows one, so the dwell is booked to this session. Otherwise omit it (Claude Code, Pi, and OpenCode fill it)."},
				},
			},
		},
	}
