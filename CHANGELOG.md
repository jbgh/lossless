...
From the 2026-09-22 live survey across Claude Code, OpenCode, and Pi (capture was healthy in all three; these are the gaps it found).

- Ask session id: since 0.1.31 only 1 of 11 Claude/Pi asks and none of about 30 OpenCode asks booked the asking session. Claude Code already hands its MCP servers `CLAUDE_CODE_SESSION_ID`; the fill now reads it when the MCP server's parent process is `claude` (every process under a Claude shell inherits the id, so a Pi or OpenCode started there does not take it; a nested `claude -p` books its own). Claude subagents share the parent's MCP server and book to the parent session, narrower than the project-wide `default` they shared before. The Pi extension (`tool_call`) and the OpenCode plugin (`tool.execute.before`) stamp the harness's own session id on every lossless ask / remember / get_record, direct or through pi-mcp-adapter's proxy (`mcp__lossless {tool,args}`, `mcp {tool,args}`, string args). A caller-sent id must have a harness shape (UUID, `ses_…`, `agent-<hex>`); `w22-ios` or `adversarial-verify-fix-delivery` no longer books a phantom session, and `<uuid>-68779` / `<uuid>.part2` trim to the UUID. Tool descriptions and the skill drop the `echo $PI_SESSION_ID` recipe: omit the id unless the prompt shows it (Grok and Codex have no fill, so there send it whenever the prompt shows it).
- Redaction blanks the secret, not the line: a secret inside a JSON line's strings becomes `[redacted]` (a token whole, a connection URL's password, a `NAME=value` value, a private key BEGIN…END block), through the end of its token so a tail outside a pattern's character class (`Sup3rS3cret!Tail#Rest`) goes too, and every other byte stays. A private key whose END is not in the same string (one string per file line in an Edit patch) drops the line. Whole-line `{"_redacted":true}` had dropped 163 lines (86 assistant turns) from six live sessions, 119 for the local dev URL `postgres://memora:memora@`; replayed over those sources, all 141 lines still flagged keep their turn with the secret blanked and none leaks. Placeholders (`<pw>`, `${PGPASSWORD}`), code references (`process.env.OPENROUTER_API_KEY`, `cookieStore.refreshToken`), secret names (`from_secret: firebase_android_api_key`), lowercase package paths (`next/font`), and paths/refspecs no longer count as secrets, for the tape or for claims. Each exemption is that exact shape: a weak snake_case password, a hyphenated passphrase, a `$`-leading password, or base64 that opens with `/` still redacts. Non-JSON lines, and lines still holding a secret after the span pass, drop whole as before.
- Extract: a guessed failure ("the push may have failed", "Either … or the failure is unrelated"), a revert offered to the user ("Revert them if you want …"), the agent's own edit tool refusing a call ("so the whole call was rejected"), and a check that would let a wrong input through read as a decision ("… instead of the album's) would still pass") no longer store, and rows of those shapes already stored stop packing (`inspect --prune` retired exactly the five on the live store). Hedged phrases are stripped before the failure check, so a failure stated beside a guess still stores, as does a revert that happened ("may have failed, so I reverted …"); "Either way, the deploy failed", "the call was rejected with a 401", and "use X, not Y: Y would break" still store. On six live transcripts only those lines changed, and a real failure took the freed cap slot.
- Project key: an origin pointing at a repo's own `.git` dir (`/Users/…/town-rescue/.git`) keyed as `town-rescue/`; it now keys `developer/town-rescue` like any local-path origin.

## 0.1.31 — 2026-09-22

- Parse: a skill body the harness injects as user text no longer extracts. Pi wraps skill docs in `<skill name=… location=…>…</skill>` (inside `<skills>`), and their rules and examples read like user constraints: `question: <the user's ask, or "what must I not forget">,` and a `Workflow \`agent()\` children often do not…` fragment both stored as constraints on the live memora tape on 2026-09-21. The wrapper's spans now strip with the rest of the harness chrome, and the strip is attribute-tolerant for every tag it handles — an attributed open like `<command-name …>`-style `<tag attr>` matched nothing before — with a tag-name boundary so `<skillful>` and a user's literal angle brackets survive. Claude's untagged `"Base directory for this skill:"` skip is unchanged.
- MCP session id: `ask` / `remember` / `get_record` book the caller's session when the call omits one and the machine has it (`LOSSLESS_SESSION_ID`, `PI_SESSION_ID`); a caller-sent `default` / `none` / `null` / `undefined` is replaced, never kept. The fill cannot cover pi on its own — pi resolves `PI_SESSION_ID` per bash command and scrubs it from inherited env, so MCP server processes never see it — so the tool descriptions and the skill now carry the deterministic recipe: the id comes from the prompt or `echo $PI_SESSION_ID` in the shell, omit only when both are empty. Every memora ask inspected from 2026-09-21 booked the `default` session behind the old "from this prompt" wording.
- Docs: write.md's skip-on-ingest list names the skill wrappers.
