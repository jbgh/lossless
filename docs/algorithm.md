...
3. **Extract durable sentences.** Classify after `Fold` (curly apostrophes). `failed` only if grounded (path, tick, or proper noun) and not meta-talk. Paths are per-sentence plus the last few *user* turns, not the whole assistant message. Nearby does not glue `auth.ts` onto a later decoy.
