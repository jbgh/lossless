...
4. **Cap 12, pathful first.** At most 5 per type. Failed dir-cap 2; decision/constraint dir-cap 5. `FilterPaths` drops abs/`~`/drive/`..`, `.git`, `node_modules`, keys, percent-encoded traversal, `tmp/` prefix, and exact `qa-report.md`. `src/tmp/…` and `docs/qa-report.md` stay. Path keys run after that filter so a dropped `/tmp` path does not leak its basename. Leading-dot dirs like `.github` stay.
